...
from .utils import normalize_proxy_payload as _normalize_proxy_payload
...
class Runner:
    ...
    async def _proxy_smoke_check(self, proxy: Any, *, url: str, timeout_s: float) -> tuple[bool, int, str]:
        ...
        impersonate_raw = _cfg(self.config, "api_http_impersonate", None)
        impersonate = None
    ...
    def _load_repair_source_records(self) -> list[dict[str, Any]]:
        ...
        try:
            ...
            accounts_file = _cfg(self.config, "accounts_file", None)
            env_path = _cfg(self.config, "env_path", None)
    ...
    async def _run_worker_with_heartbeat(
        ...
        *,
        ...
        lease_id = str(account.get("lease_id") or "").strip()
        ttl_s = max(1, int(_cfg(self.config, "lease_ttl_s", 120)))
        heartbeat_every_s = max(0.0, float(_cfg(self.config, "lease_heartbeat_s", 30.0)))
    ...
    async def _search_worker(
        ...
        *,
        ...
        max_task_attempts = max(1, int(_cfg(self.config, "max_task_attempts", 3)))
        max_fallback_attempts = max(1, int(_cfg(self.config, "max_fallback_attempts", 3)))
        max_account_switches = max(0, int(_cfg(self.config, "max_account_switches", 2)))
        configured_max_empty_pages = self._coerce_max_empty_pages(
            _cfg(self.config, "max_empty_pages", 1),
            default=1,
        )
        ...
        try:
            ...
            proxy_check_on_lease = bool(_cfg(self.config, "proxy_check_on_lease", False))
            if proxy_check_on_lease and account_session is not None:
                ...
                if has_proxy:
                    proxy_check_url = str(
                        _cfg(self.config, "proxy_check_url", "https://api.ipify.org?format=json") or ""
                    ).strip() or "https://api.ipify.org?format=json"
                    proxy_check_timeout_s = float(_cfg(self.config, "proxy_check_timeout_s", 10.0))
